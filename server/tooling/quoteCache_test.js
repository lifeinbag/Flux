...
// Standalone WS → DB updater for ALL locked AccountSets.
...
async function handleTick(token, terminal, tick) {
  ...
  if (entries.length === 0) {
    ...
    if (nowMs() - last > 5000) {
      logger.warn(`[UNMATCH] No AS entry for token=${(token || '').slice(0, 8)}… symbol=${tick.symbol}`);
  ...
  for (const ent of entries) {
    ...
    if (asCache.future && asCache.spot) {
      if (nowMs() - asCache.lastPremiumAt >= Number(process.env.PREMIUM_COLLECTION_INTERVAL)) {
        ...
        logger.info(
          `[PR] AS=${asId} ${asCache.future.symbol}↔${asCache.spot.symbol} → ` +
...
(async () => {
  try {
    ...
    console.log(`🚀 Starting WS→DB for ${sets.length} locked account sets`);
    logger.info(`🚀 Starting WS→DB for ${sets.length} locked account sets`);
    ...
    for (const set of sets) {
      ...
      const line = `AS ${set.name || set.id}: ${company1}/${sym1} ↔ ${company2}/${sym2} → premium table "${set.premiumTableName}"`;
      console.log(line);
      logger.info(line);
    }
