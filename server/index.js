...
syncDatabase()
  .then(async () => {
    ...
    // ✅ HARD-GATED QUOTE SERVICE SELECTION: Prevent double pipelines
    const quoteSource = (process.env.QUOTES_SOURCE || 'rest').toLowerCase();
    console.log(`🎯 Quote source selected: ${quoteSource.toUpperCase()}`);
    
    if (quoteSource === 'ws') {
      // WebSocket mode - ONLY schema initialization, NO REST loops
      try {
        await persistentDataCollectionService.initSchema();
        console.log('✅ Database schema ready for WebSocket mode');
      } catch (err) {
        console.error('❌ Failed to initialize database schema:', err.message);
      }
    } else {
      // REST mode - Full initialization with API polling
      try {
        await persistentDataCollectionService.initializeRest();
        const activeCollections = persistentDataCollectionService.getCollectionStatus();
        console.log(`✅ REST data collection initialized with ${activeCollections.length} active collections`);
      } catch (err) {
        console.error('❌ Failed to initialize REST data collection:', err.message);
      }
    }
    ...
    // ✅ HARD-GATED QUOTE ENGINE STARTUP: Only one active at a time
    if (quoteSource === 'ws') {
      try {
        await wsQuoteService.start();
        console.log('✅ WebSocket quote service started - NO REST quote loops');
      } catch (err) {
        console.error('❌ Failed to start WebSocket quote service:', err.message);
      }
    } else {
      try {
        await restQuoteService.start();
        console.log('✅ REST quote service started - NO WebSocket quote loops');  
      } catch (err) {
        console.error('❌ Failed to start REST quote service:', err.message);
      }
    }
