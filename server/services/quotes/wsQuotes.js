...
// ---------- Config ----------
// ✅ BULLETPROOF HEARTBEAT: Support both *_INTERVAL and *_SEC environment variables
const HEARTBEAT_MS = (() => {
  const ms  = Number(process.env.WS_HEARTBEAT_INTERVAL);
  const sec = Number(process.env.WS_HEARTBEAT_SEC);
...
const IDLE_TIMEOUT_MS = (() => {
  const ms  = Number(process.env.WS_IDLE_TIMEOUT_MS);  
  const sec = Number(process.env.WS_IDLE_TIMEOUT_SEC);
  if (Number.isFinite(ms)  && ms  > 0) return ms;
  if (Number.isFinite(sec) && sec > 0) return sec * 1000;
  ...
  try {
    ...
    c.ws.on('open', () => {
      ...
      c.idleTimer = setInterval(() => {
        ...
        if (silent > IDLE_TIMEOUT_MS / 1000) {
